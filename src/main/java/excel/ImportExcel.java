...
public class ImportExcel {
	...
	public ImportExcel(String fileName, InputStream is, int headerNum, int sheetIndex) throws InvalidFormatException, IOException {
		if (StringUtils.isBlank(fileName)) {
			throw new BusinessException(2, "导入文档为空!");
			...
			throw new BusinessException(2, "文档格式不正确!");
		}
		if (this.wb.getNumberOfSheets() < sheetIndex) {
			throw new BusinessException(2, "文档中没有工作表!");
		}
	...
	public String getCellValueForString(Row row, int column) {
		...
		if (cell != null) {
			if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
				...
				if (StringUtils.isNotBlank(dataFormatString) && "m/d/yy".equals(dataFormatString)) {
					return this.formatDate(cell.getDateCellValue());
				}
	...
	public <E> List<E> getDataList(Class<E> cls, int... groups) throws InstantiationException, IllegalAccessException {
		...
		for (int i = this.getDataRowNum(); i < this.getLastDataRowNum(); i++) {
			...
			dataList.add(e);
			logger.info("Read success: {} ", sb.toString());
		}
		return dataList;
	}
